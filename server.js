...
const state = {
  version: "4.1.2-STATE-MACHINE-INDEX-FIX",
...
function connectFeed(){
  ...
  getAccessToken().then(token=>{
    ...
    dhanWs.on("open",()=>{state.dhanConnected=true;state.server.reconnects++;state.feedLastMessageAgeMs=state.feedLastMessage?now()-state.feedLastMessage:null;state.feedStale=true;log("Dhan live feed connected");subscribe(dhanWs,state.subscriptions.length?state.subscriptions:[{ExchangeSegment:underlyingInfo().feedSegment,SecurityId:underlyingInfo().securityId}],21);broadcast({type:"state",state:publicState()});});
...
server.listen(PORT,"0.0.0.0",()=>{log(`Bharati Unique Backend V4.1.1 listening on 0.0.0.0:${PORT}`);boot();});
