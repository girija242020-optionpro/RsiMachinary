...
const NIFTY50_VOLUME_BROADCAST_MS = Math.max(100, Number(process.env.NIFTY50_VOLUME_BROADCAST_MS || 250));
const NIFTY50_SYMBOLS = String(process.env.NIFTY50_SYMBOLS || "ADANIENT,ADANIPORTS,APOLLOHOSP,ASIANPAINT,AXISBANK,BAJAJ-AUTO,BAJFINANCE,BAJAJFINSV,BEL,BHARTIARTL,CIPLA,COALINDIA,DRREDDY,EICHERMOT,ETERNAL,GRASIM,HCLTECH,HDFCBANK,HDFCLIFE,HINDALCO,HINDUNILVR,ICICIBANK,ITC,INDIGO,JSWSTEEL,JIOFIN,KOTAKBANK,LT,M&M,MARUTI,MAXHEALTH,NTPC,NESTLEIND,ONGC,POWERGRID,RELIANCE,SBILIFE,SHRIRAMFIN,SBIN,SUNPHARMA,TCS,TATACONSUM,TMPV,TATASTEEL,TECHM,TITAN,TRENT,ULTRACEMCO,WIPRO,INFY").split(",").map(x=>x.trim()).filter(Boolean);
...
const state = {
  version: "4.1.4-STATE-MACHINE-NIFTY50-VOLUME",
  ...
  subscriptions: [],
  server: { startedAt: Date.now(), ticks: 0, packets: 0, reconnects: 0, depthPackets: 0, nifty50VolumePackets: 0, nifty50VolumeShares: 0, nifty50Constituents: 0 },
};
...
const nifty50Constituents = new Map();
let nifty50VolumePending = 0;
let nifty50VolumeTimer = null;
let nifty50LastVolumeFlush = 0;
...
async function loadInstrumentMaster(){
  ...
  rebuildNifty50Constituents();
  log(`Loaded ${instruments.size} Dhan instruments`);
}

function normalizeSymbol(s){return String(s||"").toUpperCase().replace(/[^A-Z0-9]/g,"");}
function rebuildNifty50Constituents(){
  nifty50Constituents.clear();
  const wanted=new Set(NIFTY50_SYMBOLS.map(normalizeSymbol));
  for(const x of instruments.values()){
    if(x.exchangeSegment!=="NSE_EQ")continue;
    const candidates=[x.symbol,x.tradingSymbol,x.customSymbol].map(normalizeSymbol).filter(Boolean);
    if(candidates.some(v=>wanted.has(v))) nifty50Constituents.set(String(x.securityId),x);
  }
  state.server.nifty50Constituents=nifty50Constituents.size;
  log(`NIFTY50 volume constituents mapped: ${nifty50Constituents.size}/${NIFTY50_SYMBOLS.length}`);
  const missing=NIFTY50_SYMBOLS.filter(w=>!nifty50Constituents.has([...instruments.values()].find(x=>x.exchangeSegment==="NSE_EQ" && [x.symbol,x.tradingSymbol,x.customSymbol].map(normalizeSymbol).includes(normalizeSymbol(w)))?.securityId||""));
  if(missing.length)log(`NIFTY50 volume missing symbols: ${missing.join(",")}`);
}
function subscribeNifty50Volume(){
  if(!dhanWs||dhanWs.readyState!==WebSocket.OPEN||!nifty50Constituents.size)return;
  const inst=[...nifty50Constituents.values()].map(x=>({ExchangeSegment:"NSE_EQ",SecurityId:String(x.securityId)}));
  subscribe(dhanWs,inst,21);
}
function flushNifty50Volume(){
  if(state.indexKey!=="NIFTY")return;
  if(nifty50VolumePending<=0)return;
  const info=INDEXES.NIFTY;
  const spot=state.spot;
  if(!Number.isFinite(spot))return;
  const delta=nifty50VolumePending;
  nifty50VolumePending=0;
  nifty50LastVolumeFlush=now();
  state.server.nifty50VolumePackets++;
  state.server.nifty50VolumeShares+=delta;
  const k=key(info.feedSegment,info.securityId);
  const t=ticks.get(k)||{securityId:info.securityId,exchangeSegment:info.feedSegment,securityIdNum:Number(info.securityId)};
  t.ltp=spot;
  t.updatedAt=now();
  t.tradeQty=delta;
  t.volumeDelta=delta;
  t.aggregateVolumeSource="NIFTY50_COMPONENTS";
  t.aggregateVolume=Number(t.aggregateVolume||0)+delta;
  t.ltt=now();
  ticks.set(k,t);
  state.lastTick=clone(t);
  const c=updateCandle(k,t);
  const ind=indicatorSnapshot(k);
  const msg={type:"tick",data:{close:spot,ltp:spot,open:spot,high:spot,low:spot,volume:delta,timestamp:now(),rsi5:ind.rsi5,rsiEma14:ind.rsiEma14,dema14:ind.dema14,volSma250:ind.volSma250,securityId:info.securityId,exchangeSegment:info.feedSegment,depth:t.depth20||null,volumeSource:"NIFTY50_COMPONENTS",volumeMode:"SUM_OF_50_COMPONENTS"}};
  for(const ws of clients)if(ws.readyState===WebSocket.OPEN)ws.send(JSON.stringify(msg));
}
function queueNifty50Volume(delta){
  const d=Number(delta||0);
  if(!(d>0))return;
  nifty50VolumePending+=d;
  if(!nifty50VolumeTimer)nifty50VolumeTimer=setTimeout(()=>{nifty50VolumeTimer=null;flushNifty50Volume();},NIFTY50_VOLUME_BROADCAST_MS);
}
...
async function syncMarketSubscriptions(rows){
  ...
  const volumeInst=[...nifty50Constituents.values()].map(x=>({ExchangeSegment:"NSE_EQ",SecurityId:String(x.securityId)}));
  const inst=[...base,...near.map(x=>({ExchangeSegment:"NSE_FNO",SecurityId:x.securityId})),...volumeInst];
  state.subscriptions=inst;
  subscribe(dhanWs,inst.filter(x=>x.ExchangeSegment!=="IDX_I"),21);
  subscribe(dhanWs,[{ExchangeSegment:underlyingInfo().feedSegment,SecurityId:underlyingInfo().securityId}],15);
  subscribeNifty50Volume();
  syncDepthSubscriptions(near.slice(0,DEPTH_MAX_INSTRUMENTS));
}
...
function decodeOne(buf){
  // Dhan v2 response header: byte 0=response code, bytes 1-2=message length, byte 3=segment, bytes 4-7=security id.
  if(buf.length<8)return null; const dv=new DataView(buf.buffer,buf.byteOffset,buf.byteLength); const code=dv.getUint8(0),seg=dv.getUint8(3),sec=dv.getUint32(4,true),len=dv.getUint16(1,true)||buf.length;
  ...
  if(code===1&&buf.length>=12)return {...base,type:"index",ltp:f(8),ltt:null};
...
function decodeMessage(data){
  const b=Buffer.from(data),out=[]; let off=0;
  while(off+8<=b.length){
    let len=b.readUInt16LE(off+1);
    if(!len || len<8){len=b.length-off;}
    if(off+len>b.length){
      // Some websocket frames contain one complete packet only; do not discard it if the reported length is inconsistent.
      if(off===0)len=b.length; else break;
    }
    const end=off+len; const x=decodeOne(b.subarray(off,end)); if(x)out.push(x);
    if(end<=off)break; off=end;
  }
  return out;
}
...
function normalizedTick(k,t){const ind=indicatorSnapshot(k);return {type:"tick",data:{close:t.ltp,ltp:t.ltp,open:t.open??t.ltp,high:t.high??t.ltp,low:t.low??t.ltp,volume:Number(t.aggregateVolumeDelta||t.tradeQty||0),timestamp:t.ltt||now(),rsi5:ind.rsi5,rsiEma14:ind.rsiEma14,dema14:ind.dema14,volSma250:ind.volSma250,securityId:t.securityId,exchangeSegment:t.exchangeSegment,depth:t.depth20||null}};}
...
function updateTick(p){
  ...
  const isNiftyConstituent=segName==="NSE_EQ"&&nifty50Constituents.has(String(p.sec));
  if(isNiftyConstituent&&state.indexKey==="NIFTY"&&Number(t.volumeDelta)>0)queueNifty50Volume(t.volumeDelta);
...
function connectFeed(){
  ...
  getAccessToken().then(token=>{
    ...
    dhanWs.on("open",()=>{state.dhanConnected=true;state.server.reconnects++;state.feedLastMessageAgeMs=state.feedLastMessage?now()-state.feedLastMessage:null;state.feedStale=true;log("Dhan live feed connected");subscribe(dhanWs,state.subscriptions.length?state.subscriptions.filter(x=>x.ExchangeSegment!=="IDX_I"):[{ExchangeSegment:underlyingInfo().feedSegment,SecurityId:underlyingInfo().securityId}],21); subscribe(dhanWs,[{ExchangeSegment:underlyingInfo().feedSegment,SecurityId:underlyingInfo().securityId}],15); subscribeNifty50Volume(); broadcast({type:"state",state:publicState()});});
...
wss.on("connection",ws=>{
  ...
  ws.on("message",raw=>{
    ...
    if(m?.type==="subscribe"){
      const sym=String(m.symbol||"NIFTY").toUpperCase(); if(INDEXES[sym]){state.indexKey=sym;state.expiry=null;clientPrefs.set(ws,{symbol:sym,timeframe:String(m.timeframe||"1m")});const x=INDEXES[sym];subscribe(dhanWs,[{ExchangeSegment:x.feedSegment,SecurityId:x.securityId}],15);if(state.lastTick&&String(state.lastTick.securityId)===x.securityId)broadcastTo(ws,normalizedTick(key(x.feedSegment,x.securityId),state.lastTick));}
      return;
    }
...
async function boot(){
  ...
  instrumentRefreshTimer=setInterval(async()=>{try{await loadInstrumentMaster();subscribeNifty50Volume();}catch(e){log("Instrument refresh:",e.message);}},6*3600_000);
...
server.listen(PORT,"0.0.0.0",()=>{log(`Bharati Unique Backend V4.1.4 listening on 0.0.0.0:${PORT}`);boot();});
