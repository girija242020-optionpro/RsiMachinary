# Bharati Unique Backend V4.1.4 — RSI DEMA VOLUME State-Machine Compatible
...
## V4.1.4 — NIFTY 50 component-volume replacement

The PWA remains unchanged. The backend now supplies its `volume` field from a TradingView-style NIFTY 50 component-volume calculation instead of relying on the non-tradable NIFTY index's own volume. TradingView documents that NIFTY index volume is calculated as the sum of the volumes of all current index components for the selected timeframe.

Implementation:
- Subscribes to the current NIFTY 50 constituent equities through Dhan Full Packet (RequestCode 21).
- Maps the 50 NSE symbols to Dhan Security IDs from the live instrument master.
- Uses each stock's cumulative volume delta to avoid double-counting repeated packets.
- Aggregates the 50 constituent volume deltas and sends the aggregate as the PWA-compatible `tick.data.volume`.
- Keeps NIFTY spot price from Security ID 13.
- Keeps RSI 5, RSI EMA 14, DEMA 14 and Volume SMA 250 on the same NIFTY candle stream.
- Aggregation is flushed every `NIFTY50_VOLUME_BROADCAST_MS` milliseconds (default 250 ms) to avoid flooding a mobile PWA.
- `/api/health` now exposes `nifty50Volume.constituents`, `packets`, `shares`, `pending`, and `lastFlush`.

This is an index-volume proxy/replication, not an exchange-traded NIFTY-index volume field. NSE itself does not publish an exchange-traded volume for the non-tradable index; TradingView uses the constituent-sum methodology for its NIFTY volume.
